...
static bool load_window(
    lua_State *L, struct module *modules, size_t n,
...
static int set_windows(lua_State *L) {
    ...
    for(lua_Integer i = 0; i != n; ++i) {
        lua_geti(L, 1, i + 1);
        if(!load_window(L, modules, n_modules, ws + i, ms + i))
            goto e1;
        lua_pop(L, 1);
    }
    *d->windows = ws;
    ...
    struct window *w, size_t **m)
{
    ...
    if(lua_getfield(L, -3, "modules") != LUA_TNIL) {
        lua_len(L, -1);
        const lua_Integer ni = lua_tointeger(L, -1);
        lua_pop(L, 1);
        size_t *const mi = checked_calloc((size_t)ni + 1, sizeof(*mi));
        if(!mi)
            return false;
        for(lua_Integer i = 0; i != ni; ++i) {
            lua_geti(L, -1, i + 1);
            struct module *const mii =
                find_module(modules, n, lua_tostring(L, -1));
            if(mii)
                mi[i] = (size_t)(mii - modules);
            lua_pop(L, 1);
        }
        mi[ni] = (size_t)-1;
        *m = mi;
